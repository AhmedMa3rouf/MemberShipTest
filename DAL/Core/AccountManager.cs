...
namespace DAL.Core
{
    public class AccountManager : IAccountManager
    {
        ...
        /*Ma3rouf*/
        public virtual async Task<ApplicationUser> GetUserAsync(ClaimsPrincipal principal)=> await _userManager.GetUserAsync(principal);

        public virtual Task<bool> CanSignInAsync(ApplicationUser User)=>  _signInManager.CanSignInAsync(User);

        public virtual Task SignInAsync(ApplicationUser user, bool isPersistent = false, string authenticationMethod = null)=> 
            _signInManager.SignInAsync(user, isPersistent, authenticationMethod);

        public virtual async Task<SignInResult> PasswordSignInAsync(string userName, string password, bool isPersistent = false, bool lockoutOnFailure = false)=> await _signInManager.PasswordSignInAsync(userName,password,isPersistent,lockoutOnFailure);

        public virtual async Task<SignInResult> PasswordSignInAsync(ApplicationUser User, string password, bool isPersistent = false, bool lockoutOnFailure = false)
        => await _signInManager.PasswordSignInAsync(User, password, isPersistent, lockoutOnFailure);

        public virtual Task SignOutAsync() => _signInManager.SignOutAsync();
        /*End Ma3rouf*/
